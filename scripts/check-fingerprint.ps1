...
# Roadcraft - Retail.exe on disk and compare it against every build profile in
...
function Get-CheckTargets {
    ...
    $config = Get-GameConfig -GameId 'roadcraft'
    $paths = @(Find-AllGamePaths -GameId 'roadcraft')
    if ($paths.Count -eq 0) { throw "RoadCraft not found. Pass -ExePath explicitly." }

    foreach ($path in $paths) {
        # Roadcraft - Retail.exe is nested under the install root, and a GDK build can
