...
# commit attached.
#
#   RoadCraftHeadTracking-v{version}-installer.zip  (GitHub, install.cmd)
#
...
# payload is a proxy DLL (dinput8.dll) plus RoadCraftHeadTracking.asi, and both
# have to sit beside Roadcraft - Retail.exe in root\bin\pc - Ultimate ASI Loader only
# ever scans the directory the exe is in. A mod manager deploys into one fixed
# subtree of the game folder, and no manager route that reaches root\bin\pc has
# been found for this game, so a Nexus archive could install "successfully"
# while the loader never loads.
...
param(
    ...
    # Escape hatch for a machine with no node. Covers the third-party notices
    # gate as well, because that one needs node too and a switch that left half
    # the validation running would be worse than one that says what it turns
    # off. Deliberately not automatic: an unvalidated manifest is exactly the
    # failure these gates exist to catch, so skipping them has to be typed.
...
$asiName = 'RoadCraftHeadTracking.asi'
...
# The banner the mod logs comes from CMakeLists, not from pixi.toml. release.ps1
# mirrors the version across, but a hand edit - or a release.ps1 that died
# between writing the file and committing - leaves the mod reporting a version
# that was never cut, in the one line a bug report quotes.
$cmakeContent = Get-Content -Raw (Join-Path $projectDir 'CMakeLists.txt')
if ($cmakeContent -notmatch 'project\(RoadCraftHeadTracking\s+VERSION\s+([0-9]+\.[0-9]+\.[0-9]+)') {
    throw 'Could not read the project VERSION from CMakeLists.txt'
}
if ($Matches[1] -ne $version) {
    throw "CMakeLists.txt says VERSION $($Matches[1]) but pixi.toml says $version. The mod would log a version that was never released."
}

if (Test-Path $releaseDir) { Remove-Item $releaseDir -Recurse -Force }
New-Item -ItemType Directory -Path $releaseDir | Out-Null

$stage = Join-Path $env:TEMP "rc-ht-stage-$([Guid]::NewGuid().ToString('N'))"
...
$installerZip = Join-Path $releaseDir "RoadCraftHeadTracking-v$version-installer.zip"
...
if ($SkipManifestValidation) {
    Write-Host "Manifest AND third-party notices validation SKIPPED at your request (-SkipManifestValidation). Both gates need node, so the one switch covers both." -ForegroundColor Yellow
} else {
    ...
    # Same gate for the notices. Assert-CoreCommitInNotices only checks the core
    # commit hash; nothing else ever ran the component check, so bumping the
    # vendored loader and forgetting its notice stayed green all the way to a
    # published release.
    $noticesValidator = Join-Path $projectDir 'cameraunlock-core\scripts\validate-notices.mjs'
    if (-not (Test-Path $noticesValidator)) { throw "$noticesValidator is missing - cannot validate THIRD-PARTY-NOTICES.md. Run ``git submodule update --init --recursive``." }
    node $noticesValidator
    if ($LASTEXITCODE -ne 0) {
        Remove-Item $installerZip -Force -ErrorAction SilentlyContinue
        throw "THIRD-PARTY-NOTICES.md does not cover what this build ships."
    }
}
}
finally {
