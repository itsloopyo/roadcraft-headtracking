...
if ($Version -eq 'nightly') {
    # Not $LASTEXITCODE: after a PowerShell script that holds the exit code of
    # the last NATIVE command the script happened to run (a git, a gh), not the
    # script's own outcome. release-nightly.ps1 runs with Stop and throws, so a
    # failure arrives as an exception; reaching the next line means it worked.
...
if ($cmakeContent -notmatch 'project\(RoadCraftHeadTracking VERSION [0-9]+\.[0-9]+\.[0-9]+') {
    throw "No project VERSION found in $cmakePath"
}
$cmakeContent = $cmakeContent -replace '(project\(RoadCraftHeadTracking VERSION )[0-9]+\.[0-9]+\.[0-9]+', "`${1}$newVersion"
