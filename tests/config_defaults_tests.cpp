...
// The values the fleet fixes, as literals. Everything else in this suite
// compares Config against a default-constructed Config, which locks the three
// sources to each other and says nothing about what they hold: a value could be
// edited in all three at once and the suite would stay green. The travel limits
// and the two smoothing values are pinned by construction, to
// cameraunlock-core's own constants. These six are bare literals in config.h
// with nothing else holding them, and the four key codes are what AGENTS.md
// fixes so that one action sits on the same key across every mod.
void CheckShippedValues() {
    const Config defaults;
    std::printf("The shipped defaults are the ones the fleet mandates\n");
    Check(defaults.udp_port == 4242, "UdpPort is the OpenTrack standard 4242");
    Check(defaults.enable_on_startup, "EnableOnStartup is on");
    Check(defaults.toggle_key == 0x23, "ToggleKey is End");
    Check(defaults.cycle_mode_key == 0x21, "CycleModeKey is Page Up");
    Check(defaults.chord_toggle_key == 0x59, "ChordToggleKey is Y, for Ctrl+Shift+Y");
    Check(defaults.chord_cycle_mode_key == 0x47, "ChordCycleModeKey is G, for Ctrl+Shift+G");
}
...
void CheckMatchesDefaults(const Config& cfg, const char* source) {
    ...
    CheckClose(cfg.limit_y_down, defaults.limit_y_down, "LimitYDown");
...
Config Poisoned() {
    ...
    cfg.limit_x = cfg.limit_y = cfg.limit_y_down = cfg.limit_z = cfg.limit_z_back = 0.45f;
    return cfg;
}
...
void CheckReferenceIniIsByteIdentical(const std::string& generated_path) {
    ...
    const std::string reference = ReadWholeFile(std::string(RC_SOURCE_DIR) + "\\HeadTracking.ini");
...
void GeneratedDefaultsTests() {
    const std::string dir = rc_test::MakeTempDir("config");
    if (dir.empty()) return;

    const std::string path = rc_test::IniPathIn(dir);
    std::printf("The generated default HeadTracking.ini\n");
    // Cleared first. A run that crashed, or a reused process id, leaves the
    // directory behind with its file still in it, and an assertion that only
    // looks after the call passes just as happily on that stale one.
    DeleteFileA(path.c_str());
    if (!Check(GetFileAttributesA(path.c_str()) == INVALID_FILE_ATTRIBUTES,
               "is not on disk before the call")) {
        rc_test::RemoveTempDir(dir);
        return;
    }
...
void ReferenceIniTests() {
    // RC_SOURCE_DIR is the repo root, where the reference HeadTracking.ini
...
int main() {
    std::printf("RoadCraft head tracking - config default tests\n");
    std::printf("=====================================================\n");
    CheckShippedValues();
    GeneratedDefaultsTests();
    ReferenceIniTests();
    return rc_test::Summary("config defaults");
}
