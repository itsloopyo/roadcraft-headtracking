...
int main() {
    std::printf("RoadCraft head tracking - hotkey remap tests\n");
    std::printf("===================================================\n");
    g_dir = rc_test::MakeTempDir("hotkey");
    if (g_dir.empty()) return rc_test::Summary("hotkey remap");
    RemapTests();
    RefusedValuesKeepThePreviousBindingTests();
    CollidingBindingsAreRefusedTests();
    rc_test::RemoveTempDir(g_dir);
    return rc_test::Summary("hotkey remap");
}
