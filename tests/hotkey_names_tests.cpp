...
const int kKeysToName[] = {
    ...
    // Near the ends of the range the config layer accepts: the mouse buttons
    // (0x01-0x06) are refused, so 0x07 is the low end and 0xFE the high one.
    // 0x07 itself is covered below, where it stands in for a key with no name.
    0x08, 0x70, 0xFE,
};
...
int main() {
    std::printf("RoadCraft head tracking - hotkey name tests\n");
    std::printf("=======================================================\n");
    EveryKeyGetsAName();
    AnUnnameableKeyFallsBackToItsCode();
    return rc_test::Summary("hotkey names");
}
