...
inline std::string MakeTempDir(const char* suite) {
    ...
    std::string dir = std::string(temp, n) + "rc-ht-" + suite + "-test-"
                    + std::to_string(GetCurrentProcessId());
    // An existing directory is fine and expected - a crashed run leaves one, and
    // a suite that cares about a stale file in it clears that file itself.
    // Anything else means there is no directory to write into, and a suite that
    // carried on would report the mod's behaviour from a path that is not there.
    if (!CreateDirectoryA(dir.c_str(), nullptr) && GetLastError() != ERROR_ALREADY_EXISTS) {
        std::printf("  FAIL: could not create %s (%lu)\n", dir.c_str(), GetLastError());
        ++g_failures;
        return {};
    }
    return dir;
